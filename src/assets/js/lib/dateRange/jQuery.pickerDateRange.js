...
function pickerDateRange(inputId, options) {
    ...
    if (this.mOpts.noCalendar) {
        ...
        $('#' + this.inputId).attr('readonly', 'readonly');
    }
    // 把时间选择框放到页面中
    // $('#' + this.inputId).insertBefore()
    $(wrapper[this.mOpts.theme].join('')).insertAfter($('#' + this.inputId));
    // $(0 < $('#appendParent').length ? '#appendParent' : document.body).append(wrapper[this.mOpts.theme].join(''));
    $('#' + this.calendarId).css('z-index', 999);
    // 初始化目标地址的元素
    ...
    $('#' + this.startDateId).bind('click', function (event) {
    ...
    // 为输入框添加click事件
    $('#' + this.inputId).bind('click', function (event) {
        __method.init();
        __method.show(false, __method);
        return false;
    });
    $('#' + this.mOpts.inputTrigger).bind('click', function (event) {
        __method.init();
        __method.show(false, __method);
        return false;
    });
    $('#' + this.mOpts.compareTrigger).bind('click', function (event) {
        __method.init(true);
        __method.show(true, __method);
        return false;
    });
    // 为输入框添加click事件
    $('#' + this.inputCompareId).bind('click', function (event) {
        __method.init(true);
        __method.show(true, __method);
        return false;
    });
    ...
    $('#' + this.compareCheckboxId).bind('click', function (event) {
    ...
    if (this.mOpts.replaceBtn && $('#' + this.mOpts.replaceBtn).length > 0) {
        ...
        $('.contrast').hide();
        $('#' + this.mOpts.replaceBtn).bind('click', function (event) {
            var self = this;
...
pickerDateRange.prototype.init = function (isCompare) {
    ...
    // 上一个月
    $('#' + this.preMonth).bind('click', function (event) {
    ...
    // 上一年
    $('#' + this.preYear).bind('click', function (event) {
...
pickerDateRange.prototype.show = function (isCompare, __method) {
    ...
    if (true == this.mOpts.singleCompare || true == this.mOpts.isSingleDay) {
        ...
        $('#' + this.mOpts.joinLineId).css('display', 'inline-block');
        $('.' + this.mOpts.joinLineId).css('display', 'inline-block');
    }
    ...
    if (0 < clientWidth && $("#" + this.calendarId).width() + pos.left > clientWidth) {
        ...
        left = $('#' + this.inputId)[0].clientWidth - $("#" + this.calendarId).width() - 1
    }
    ...
    // $("#" + this.calendarId).css('top', pos.top + offsetHeight + 'px');
    //第一次显示的时候，一定要初始化输入框
...
pickerDateRange.prototype.setValue = function (data) {
    if(data){
        $('#' + this.startDateId).val(data.startDate);
        $('#' + this.endDateId).val(data.endDate);
        this.close(1);
        if(this.mOpts.shortOpr === true){
            $('#' + this.inputId).val(data.startDate);
        }else{
            $('#' + this.inputId).val(data.startDate + this.mOpts.defaultText + data.endDate);
        }
    }else{
        this.close(1);
