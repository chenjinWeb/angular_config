...
(function ($) {
    ...
    $.extend($.validator.defaults, {
        ...
        highlight: function (element, errorClass) {
            ...
            _$label = $(_$p).find("label[for=" + element.id + "]");
			$(element).addClass("error-input");
			_$label.addClass("error-message");
            //$("<i class='icon-error'></i>").prependTo(_$label);

            $(element).nextAll(".info-message").hide();
        },
        onchange: function (element, event) {
            if ($(element).is(":hidden")) {
                if (element.name in this.submitted || element.value)
                    this.element(element);
            }
        },
        success: function (label, element) {
            label.html("<i class='icon-success'></i>");
            var _$p = $(element).parent();
	...
	$.validator.addMethod("telphone", function (value, element, param) {
		return this.optional(element) || /((\d{11})|^((\d{7,8})|(\d{4}|\d{3})-(\d{7,8})|(\d{4}|\d{3})-(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1})|(\d{7,8})-(\d{4}|\d{3}|\d{2}|\d{1}))$)/.test(value) || /^(0|86|17951)?(13[0-9]|15[012356789]|17[678]|18[0-9]|14[57])[0-9]{8}$/.test(value);
	}, "请输入正确的联系电话");
	$.validator.addMethod("pwd", function (value, element, param) {
        return (value && /[0-9]{1,}/.test(value) && /[a-zA-Z]{1,}/.test(value));
    }, "密码必须包含字符和数字");
